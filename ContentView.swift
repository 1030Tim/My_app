    ...
    var body: some View {
        ScrollView{
            ...
            Text("關於").font(.largeTitle).fontWeight(.heavy) .foregroundColor(Color.orange).underline().italic()+Text("我的")+Text("撐").font(.largeTitle).foregroundColor(.red)+Text("竿跳高").font(.title3)
                .bold().font(.title)
            HStack{
                Text(me.seep)+(Text("全中錦第三").foregroundColor(.purple).font(.system(size: 22)))
            ...
            HStack{
                Image(me.gogo)
                    .resizable().aspectRatio(contentMode: .fit)
                    .frame(width:250)
                Text("第一次參加資訊類比賽，第一次使用Ｃ語言比賽，還要跟友心大姐多多學習")
                    .font(.system(size: 14))
            }
            VStack{
                Text("-------------------------------------------------")
            ...
            HStack{
                Image(systemName: "person.fill")
                    .foregroundColor(.secondary)
                Text("開發者：")+(Text("陳閔駿").foregroundColor(.brown).font(.title3))
